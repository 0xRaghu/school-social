...
const validateCompetitionsInput = require('../../validation/competitions');
const validateAchievementsInput = require('../../validation/achievements');
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      const newComp = {
        ...
        opentilllast: req.body.opentilllast,
...
app.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Add to exp array
      profile.achievements.unshift(newAch);
...
app.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        // Get remove index
        const removeIndex = profile.competitions
          .map(item => item.id)
          .indexOf(req.params.comp_id);

        // Splice out of array
        profile.competitions.splice(removeIndex, 1);
...
app.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        // Get remove index
        const removeIndex = profile.achievements
          .map(item => item.id)
